...
- Цвета берутся из `tg.colorScheme` и `tg.themeParams` — интерфейс автоматически переключается в тёмную тему, если Telegram работает в тёмном режиме.
...
## Запуск из внешних приложений

**Открытие конкретного объекта**

Чтобы из другого приложения (например, клиентского кабинета или бота) открыть карточку объекта, передайте его id в query-параметре:

```
https://minitest.bitrixabd.ru/inventory/?storeId=3751
```

Приложение автоматически загрузит этот объект через API и откроет его карточку.

Если объект не найден — отображается toast-ошибка, пользователь остаётся на главном экране.
